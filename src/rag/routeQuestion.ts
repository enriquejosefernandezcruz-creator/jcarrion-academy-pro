// ==============================
// FILE: src/rag/routeQuestion.ts (FULL REPLACE)
// ==============================
...
// ✅ PRUEBA DE VIDA (temporal): si NO sale en consola, no estás usando este fichero
console.log("✅ ROUTEQUESTION LOADED - vTEST_001");
...
// Alias/jerga -> canónico (normalizados)
const ALIASES: Array<{ alias: string; canon: string }> = [
  { alias: "respostar", canon: "repostar" }, // typo frecuente
  { alias: "respotar", canon: "repostar" },  // typo frecuente
...
export function routeQuestion(question: string): RouteResult {
  ...
  // Intención de localización/listado (prioridad GASOLINERAS si hay señales de gas)
  ...
  // Si el usuario pide "dónde/listado/ubicación" y hay señales de gas -> GASOLINERAS.
  if (locationHits > 0 && gasHits > 0) {
    console.log("ROUTE DEBUG (precedencia GAS)", {
      normalized: q0,
      expanded,
      gasHits,
      manualHits,
  ...
  // Scoring
  const gasScore = gasHits * 2 + locationHits; // intención suma
  const manualScore = manualHits * 2;

  // Ambigüedad: mezcla de dominios (sin intención clara de localización)
